...
export default class TimeGrid extends Component {
    ...
    render() {
        ...
        events.forEach(event => {
            if (inRange(event, start, end, this.props)) {
                ...
                if (get(event, allDayAccessor) || !dates.eq(eStart, eEnd, 'day') || (dates.isJustDate(eStart) && dates.isJustDate(eEnd))) {
        ...
        return (
            <div className='rbc-time-view'>
                ...
                <div ref='content' className='rbc-time-content'>
                    ...
                    <TimeColumn {...this.props} showLabels style={{
                        width
                    }} ref={gutterRef} className='rbc-time-gutter'/> {this.renderEvents(range, rangeEvents, this.props.now)
}
                </div>
            </div>
        );
    ...
    renderEvents(range, events, today) {
        ...
        return range.map((date, idx) => {
            let daysEvents = events.filter(event => dates.inRange(date, get(event, startAccessor), get(event, endAccessor), 'day'))

            return (<DayColumn {...this.props } min={dates.merge(date, min)} max={dates.merge(date, max)} eventComponent={components.event} className={cn({
                'rbc-now': dates.eq(date, today, 'day')
            })} style={segStyle(1, this._slots)} key={idx} date={date} events={daysEvents}/>)
        })
    }
    ...
    renderAllDayEvents(range, levels) {
        ...
        return levels.map((segs, idx) => <EventRow eventComponent={this.props.components.event} titleAccessor={this.props.titleAccessor} startAccessor={this.props.startAccessor} endAccessor={this.props.endAccessor} allDayAccessor={this.props.allDayAccessor} eventPropGetter={this.props.eventPropGetter} selected={this.props.selected} onSelect={this._selectEvent} slots={this._slots} key={idx} segments={segs} start={first} end={last}/>)
    }

    renderHeader(range, segments, width) {
        let {messages, rtl, onSelectSlot, hideAllDay} = this.props;
        ...
        let allDaySlotClass = cn({
            'rbc-row': true,
            'hide': hideAllDay
        });

        return (
            <div ref='headerCell' className={cn('rbc-time-header', isOverflowing && 'rbc-overflowing')} style={style}>
                <div className='rbc-row'>
                    <div className='rbc-label rbc-header-gutter' style={{
                        width
                    }}/> {this.renderHeaderCells(range)}
                </div>
                <div className={allDaySlotClass}>
                    <div ref={ref => this._gutters[0] = ref} className='rbc-label rbc-header-gutter' style={{
                        width
                    }}>
                        {message(messages).allDay}
                    </div>
                    <div ref='allDay' className='rbc-allday-cell'>
                        <BackgroundCells slots={range.length} container={() => this.refs.allDay} selectable={this.props.selectable} onSelectSlot={handleSelectSlot}/>
                        <div style={{
                            zIndex: 1,
                            position: 'relative'
                        }}>
    ...
    renderHeaderCells(range) {
        ...
        return range.map((date, i) => <div key={i} className='rbc-header' style={segStyle(1, this._slots)}>
